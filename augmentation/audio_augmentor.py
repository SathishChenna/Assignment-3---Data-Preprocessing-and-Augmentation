...
class AudioAugmentor:
    ...
    def apply_operation(self, operation: str, audio_data: np.ndarray, sr: int) -> tuple[np.ndarray, int, str, str]:
        if operation in self.operations:
            ...
            time_domain = self.generate_waveform(augmented_audio, augmented_sr)
            freq_domain = self.generate_spectrogram(augmented_audio, augmented_sr)
            return augmented_audio, augmented_sr, time_domain, freq_domain
